    ...
    const void* params)
{
  ...
  const psimd_f32 vone_cutoff = psimd_splat_f32(0x1.154244p+4f);
  ...
  for (; n >= 24 * sizeof(float); n -= 24 * sizeof(float)) {
    ...
    vf0123 = psimd_blend_f32(vx0123 > vone_cutoff, vone, vf0123);
    vf4567 = psimd_blend_f32(vx4567 > vone_cutoff, vone, vf4567);
    vf89AB = psimd_blend_f32(vx89AB > vone_cutoff, vone, vf89AB);
    vfCDEF = psimd_blend_f32(vxCDEF > vone_cutoff, vone, vfCDEF);
    vfGHIJ = psimd_blend_f32(vxGHIJ > vone_cutoff, vone, vfGHIJ);
    vfKLMN = psimd_blend_f32(vxKLMN > vone_cutoff, vone, vfKLMN);
    ...
    vfKLMN = psimd_andnotmask_f32(vxKLMN < vdenorm_cutoff, vfKLMN);
  ...
  for (; n >= 4 * sizeof(float); n -= 4 * sizeof(float)) {
    ...
    // For inputs above 1.0 cutoff, replace output with 1.0.
    // Note that for NaN inputs, comparison result is false, and outputs are left unchanged.
    vf = psimd_blend_f32(vx > vone_cutoff, vone, vf);
